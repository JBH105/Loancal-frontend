...
const InfoGrid = ({loanRecord}:any) => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                const response = await fetch(`${BaseURL}/api/search-payments-by-loan-id?loan_id=${loanRecord.LoanId || 1}`);
    ...
    const updatePayment = async (event: any) => {
        try {
            ...
            const response = await fetch(`${BaseURL}/api/update-payment?payment_id=${payid}`, {
                method: 'PUT',
